...
public class MidLevel extends GuiScreen {
	
	private Level prevLevel;
	
	public MidLevel(Level l) {
		super();
		this.prevLevel = l;
		this.getButtons().add(new GuiButton("Continue", 300, 280, 200, 40, 0xff0000ff));
		this.getButtons().add(new GuiButton("Return to Menu", 300, 360, 200, 40, 0xff0000ff));
	}
	...
	public void render() {
        ...
        FontRenderer.drawStringWithColor("Level Completed", 400 - (FontRenderer.getStringWidth("Level Completed")/2), 200, 0xff1111ff);
        String next = "Next Level = " + (this.prevLevel.getLevelNumber() + 1);
	...
	public void check() {
		for(GuiButton b : this.getButtons()) {
			if(b.getButtonState()) {
				...
				case("Continue"):
					GameLoop.currentScreen = new InGame(this.prevLevel);
					break;
				}
			}
		}
