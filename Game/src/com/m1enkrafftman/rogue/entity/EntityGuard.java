...
public class EntityGuard extends EntityEnemy {

	private Step currentStep;
	public int animationTime;

	public EntityGuard(int minX, int minY, int width, int color) {
		super(minX, minY, width, color);
		this.currentStep = new Step(false);
		this.animationTime = 1;
		//TODO: Change this
		Random rand = new Random();
		this.setSpeed(rand.nextInt(3) + 3);
	}
	...
	public void renderEnemy(EntityPlayer p) {
		...
		if(this.getAttackInt() < 5) {
			...
			this.renderHealth();
			GL11.glRotatef(-this.getRotation(), 0, 0, 1);
			Cache.IMAGE_HANDLER.renderImage32(Cache.IMAGE_HANDLER.getTextureByName("guardAttack"), -16, -16);
			GL11.glRotatef(this.getRotation(), 0, 0, 1);
			this.renderHealth();
			GL11.glDisable(GL11.GL_TEXTURE_2D);
			GL11.glPopMatrix();
		...
		GL11.glRotatef(-this.getRotation(), 0, 0, 1);
		if(this.currentStep.isLeft()) {
			Cache.IMAGE_HANDLER.renderImage32(Cache.IMAGE_HANDLER.getTextureByName("guardLeft"), -16, -16);
		}else {
			Cache.IMAGE_HANDLER.renderImage32(Cache.IMAGE_HANDLER.getTextureByName("guardRight"), -16, -16);
		}
		GL11.glRotatef(this.getRotation(), 0, 0, 1);
		this.renderHealth();
		GL11.glDisable(GL11.GL_TEXTURE_2D);
	...
	public void renderHealth() {
		...
		GL11.glRecti(-(width/2), -20, width, -18);
